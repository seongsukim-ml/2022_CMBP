#ifndef ____AA_Wolff____
#define ____AA_Wolff____ 
...
class AA_Wolff{
    ...
    public:
        ...
        static string Name(){return "AA_Wolff";}
        AA_Wolff(int Lx, int Ly, int bin, double B, double Jx, double Jy, double alpha, double Tsrt, double Tfin, bool isTinf);
        AA_Wolff(vector<double> args);
        ~AA_Wolff(){
            // delete e2d;
            __finish__ = clock();
            cout << "------------------------------------------------------------------------------------------------------------------\n";
...
AA_Wolff::AA_Wolff(int Lx, int Ly, int bin, double B, double Jx, double Jy, double alpha, double Tsrt, double Tfin, bool isTinf)
...
AA_Wolff::AA_Wolff(vector<double> args):
AA_Wolff(args[0],args[1],args[2],args[3],args[4],args[5],args[6],args[7],args[8],args[9]){}

long double AA_Wolff::Prob(double delta){
    return expl(-this->cur_beta*delta);
}

void AA_Wolff::Initialize(double beta){
    this-> Calc_call = 0;
    this-> Fliped_Step = 0;
    this-> Total_Step = 0;
...
void AA_Wolff::Measure_fast(){
    // double res[] = {HH,sigma,staggered};
    // return vector<double>(res,res +sizeof(res)/sizeof(res[0]));
    return; // do nothing
}

void AA_Wolff::Calculate(int _n, bool Random){ //O(N^2)
    // 1) Select random site i
    ...
    // step 1.
    int st =  dis(gen)*N; // Select random site
    queue<int> que;
    que.push(st);
    fliped[st] = true;
    while(!que.empty()){
...
void AA_Wolff::IterateUntilEquilibrium(int equil_time,bool random){
    for(int j = 0; j < equil_time; j++)
        Calculate(0,random);
}

#endif // ____AA_Wolff____ 