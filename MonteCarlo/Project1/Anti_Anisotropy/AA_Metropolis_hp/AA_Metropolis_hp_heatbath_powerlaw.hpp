...
void AA_Metropolis::Measure(){ //O(N^2)
    ...
    for(i = 0; i < N; i++){
        ...
        for(j = i+Lx; j < N; j+=Lx) // y 방향 long range 계산
            partial_sum += e2d.power_law1D(i,j)*sc[j];
        partial_sum *= Jy;
...
void AA_Metropolis::Heatbath_Measure(){ //O(N^2)
    ...
    for(i = 0; i < N; i++){
        ...
        for(int jj = i%Lx; jj < N; jj+=Lx)  // Long range diff
            partial_sum += e2d.power_law1D(jj,i)*sc[jj];
        partial_sum *= Jy;
        // cout << partial_sum << " ";
    ...
    for(i = 0; i < n; i++){
        ...
        if((delta <= 0) || (logl(dis(gen)) < -cur_beta*delta)){
            ...
            for(INT1 jj = k%Lx; jj < N; jj+=Lx){
                heatbath[jj] += Jy*e2d.power_law1D(jj,k)*2*sc[k];
            }  // Long range diff
...
void AA_Metropolis::CalculateCorrelation(){
    ...
    for(int i = 0; i < 1; i++){
        ...
        cblas_sspr(CblasRowMajor,CblasUpper,Lx,1,AA,1,cor_short);
    }
    for(int i = 0; i < 1; i++){
        for(int j = 0; j < Ly; j++){
            BB[j] = sc[j*Lx + i];
        }
        cblas_sspr(CblasRowMajor,CblasUpper,Ly,1,BB,1,cor_long);
    }
}
