...
int main(){
    ...
    for(int i = 0; i < bin; i++){ // i is bin
        ...
        epoch = 20000;
        int M = 200;
        vector<double> CbsHH2(epoch/M);
        vector<double> CbsHH(epoch/M);
        for(int j = 0; j < epoch; j++){ // j is epoch
            ...
            HH = value[0];
            sigma = value[1];
            CbsHH2[j/M] += HH/(double)(M*M)*HH;
            CbsHH[j/M] += HH/(double)(M*M);         
        ...
        for(int j = 0; j < Nb; j++){
            double ci1 = 0, ci2 = 0;
            for(int k = 0; k < M; k++){
                int l = epoch/M*dis(gen);
                ci1 += CbsHH2[l];
                ci2 += CbsHH[l];
            }
            cbs[i] += ((beta*beta)/N*(ci1 - ci2*ci2) -c[i])*((beta*beta)/N*(ci1 - ci2*ci2) -c[i]);
        }
        cbs[i] = sqrt(cbs[i]/(Nb-1));
        cout << m[i] << " " << c[i] << " " << cbs[i] << " "<< Fliped_Step << " " << Total_Step << '\n';
    }
    ...
    if(save){
        ...
        for(int i = 0; i <bin; i++){
            ...
            temp = temp + to_string(res[i][3]) + "," + to_string(res[i][4]) + "," + to_string(cbs[i]) + "\n";
